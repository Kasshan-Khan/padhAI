...
function Navbar() {
  ...
  const [theme, setTheme] = useState(localStorage.getItem("theme") || "padhai-dark");
  ...
  const toggleTheme = () => {
    setTheme(theme === "padhai-light" ? "padhai-dark" : "padhai-light");
  };
  ...
  return (
    <div className={`navbar fixed top-0 z-50 transition-all duration-300 px-4 lg:px-8 ${scrolled || !isHomePage ? "bg-base-100/90 backdrop-blur-md shadow-md text-base-content" : "bg-transparent text-primary-content"}`}>
      <div className="navbar-start">
        ...
        <Link to="/" className="btn btn-ghost text-xl hover:bg-transparent">
          <img
            src={(theme === "padhai-dark" || (isHomePage && !scrolled)) ? padhAI_logo_white : padhAi_logo_color}
            alt="PadhAI Logo"
      ...
      <div className="navbar-end gap-2">
        <label className="swap swap-rotate btn btn-ghost btn-circle">
          <input type="checkbox" onChange={toggleTheme} checked={theme === "padhai-dark"} />
          {/* sun icon */}
        ...
        <button onClick={handleAuthAction} className="btn btn-primary rounded-full px-6 shadow-md shadow-primary/20">
          {token ? "Logout" : "Login"}
        </button>
      </div>
    </div>
